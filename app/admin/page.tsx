  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        setRecentProjects(projectData.slice(0, 5))
        
        // Update stats
        setStats({
          totalProjects: projectData.length,
          publishedProjects: projectData.filter((p: any) => p.status === 'published').length,
          totalMessages: 0, // Will be implemented when messages are added
          unreadMessages: 0, // Will be implemented when messages are added
        })
      } catch (error) {
        console.error('Failed to load stats:', error)
        setRecentProjects([])
    ...
    <div className="space-y-6">
        ...
        {statCards.map((card, i) => (
          <Link href={card.link} key={i}>
            <motion.div
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className={`bg-gradient-to-br ${card.color} rounded-2xl p-5 border ${card.border} cursor-pointer transition-all duration-300`}
            >
              {isLoading ? (
                <div className="h-8 w-16 bg-white/10 rounded animate-pulse mb-2" />
              ) : (
      ...
      <div className="bg-white/5 rounded-2xl border border-white/5 overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <h2 className="text-white font-semibold text-lg">Recent Projects</h2>
          <Link href="/admin/projects">
            <span className="text-[#2563EB] text-sm hover:underline cursor-pointer">View all</span>
          </Link>
        </div>
        <div className="divide-y divide-white/5">
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-400">
              <p>No projects yet</p>
              <Link href="/admin/projects/new">
                <span className="text-[#2563EB] text-sm mt-2 inline-block hover:underline cursor-pointer">
        ...
        {[
          ...
          <Link href={action.href} key={i}>
            <motion.div
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="bg-white/5 rounded-2xl p-5 border border-white/5 hover:border-[#2563EB]/30 cursor-pointer transition-all duration-300"
            >
              <p className="text-white font-semibold">{action.label}</p>
              <p className="text-gray-400 text-sm mt-1">{action.desc}</p>
            </motion.div>
